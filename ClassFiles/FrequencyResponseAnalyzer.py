...
class FrequencyResponseAnalyzer:
    ...
    def save_bode_plot(self, frequencies, frequency_response, output_dir="./output"):
        ...
        # Magnitude plot
        plt.subplot(2, 1, 1)
        plt.semilogx(frequencies, gain, label='System', linewidth=3.0)
        plt.title('Bode Plot')
        plt.ylabel('Magnitude [dB]')
        plt.grid(which='both', linestyle='-', linewidth='1.0')
        plt.xlim([20, 20000])
        plt.gca().xaxis.set_major_locator(ticker.LogLocator(base=2, numticks=12))
        plt.gca().xaxis.set_major_formatter(ticker.FuncFormatter(custom_formatter))

        # Phase plot
        plt.subplot(2, 1, 2)
        plt.semilogx(frequencies, phase, label='System', linewidth=3.0)
        ...
        plt.close()
        print(f"Bode plot saved as '{output_file}'.")
